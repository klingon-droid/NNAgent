...
export class OllamaProvider implements AIProvider {
  ...
  private async downloadModel(model: string): Promise<void> {
    terminalStore.addOutput([
      { text: 'MODEL DOWNLOAD', type: 'header', color: 'text-cyan-400' },
      { text: '-------------', type: 'separator', color: 'text-pink-500/30' },
      { text: `Downloading ${model}...`, type: 'text', color: 'text-pink-500' },
      { text: 'This may take a few minutes.', type: 'text', color: 'text-pink-500' }
    ]);
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (data.status === 'downloading') {
            terminalStore.addOutput([{
              text: `Downloading: ${Math.round(data.completed / 1024 / 1024)}MB / ${Math.round(data.total / 1024 / 1024)}MB`,
              type: 'progress',
              color: 'text-cyan-400'
            }]);
          }
      ...
      terminalStore.addOutput([{ text: 'Download complete!', type: 'success', color: 'text-green-400' }]);
    } catch (error) {
